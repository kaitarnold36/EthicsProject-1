...
```{r logisticRegression, warning=FALSE, error=TRUE, message=FALSE, echo = FALSE}
...
Let's try another method called Decision Tree. The Decision Tree algorithm belongs to the family of supervised learning algorithms; it can be used for solving both regression and classification problems. The general motivation for using a decision tree is to predict the class or value of target variables by learning decision rules inferred from prior data (the training data). The decision tree algorithm tries to solve the problem, by using a tree representation. Each  internal node of the tree corresponds to an attribute and each leaf node corresponds to a class label. In R there are
several packages available to run the decision tree algorithm. Here we’ll use ` rpart` package and also ` caret`
package implement the decision tree with cross-validation.
...
Let's use this Decision tree to predict the response.
...
So using Decision Tree we got around 96.1% accuracy which is even better than Logistic Regression.
...
```{r randForest, warning=FALSE, error=TRUE, message=FALSE, echo=FALSE}
# set the seed for consistent results
set.seed(1842)
...
```{r rfImportance, warning=FALSE, error=TRUE, message=FALSE, echo = FALSE}
...
```{r rfPredict, warning=FALSE, error=TRUE, message=FALSE, echo=FALSE}
...
```{r knnModel, warning=FALSE, error=TRUE, message=FALSE, echo=FALSE}
...
```{r knnCaret, warning=FALSE, error=TRUE, message=FALSE, echo=FALSE}
# set the seed for consistent results
set.seed(1842)
...
```{r tuneKnn, warning=FALSE, error=TRUE, message=FALSE, echo=FALSE}
